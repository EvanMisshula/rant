...
print("There are df1 with %d records on the %s machine" % (len(df1.index), platform.system()) )
f1.write("There are df1 with %d records on the %s machine. \n" % (len(df1.index), platform.system()) )

last_page_num = len(df1.index)/ 20
forum_page = last_page_num + 1
...
nextForumURL = 'http://theerant.yuku.com/forums/58/THEE-RANT/THEE-RANT?page=' + str(forum_page)
...
#response = mySpider.get_response()
    ...
    f1.write("There are %d topic titles.\n" % num_pageTopics)    
    ...
    f1.write("saved df1 with %d records.\n" % len(df1.index))    
    f1.flush()
    
print("Loop finished. Gathered %d pages of posts.\n" % forum_page)
f1.write("Loop finished. Gathered %d pages of posts.\n" % forum_page)
f1.close()
conn.close()
db.dispose()
